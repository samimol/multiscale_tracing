...
class RecurrentNetwork():
    ...
    def __init__(self, n_input_features,grid_size,big_pixels_size,bigger_pixels_size,device,feedforward_curve,feedforward_object,one_scale):
        self.beta = 0.02
        ...
        if self.one_scale:
            self.low_scale_layer = HiddenLayer(self.n_input_features, self.n_hidden_features, 6,self.big_pixels_size,grid_size,upper_ymod = False)
        else:
        ...
        self.output_layer = OutputLayer(self.n_hidden_features, self.n_input_features, 1,self.grid_size,self.big_pixels_size,self.bigger_pixels_size,self.one_scale)
    ...
    def do_step(self, input_env, reward, reset_traces, device):
        ...
        while (i < self.duration and norm > 0) or flag:
            ...
            if self.one_scale:
                (self.Y2mod,self.VIP2,self.SOM2) = self.low_scale_layer.forward(self.low_scale, lower_ymod=self.Xmod,upper_ymod=None,horiz = self.Y2mod)
            else:
        ...
        if self.one_scale:
            (self.Y2mod3,self.VIP23,self.SOM23) = self.low_scale_layer.forward(self.low_scale, lower_ymod=self.Xmod,upper_ymod=None,horiz = self.Y2mod)
        else:
        ...
        if self.one_scale:
            (self.Y2mod2,self.VIP22,self.SOM22) = self.low_scale_layer.forward(self.low_scale, lower_ymod=self.Xmod2,upper_ymod=None,horiz = self.Y2mod3)
        else:
        ...
        ([self.Y2mod2d,self.VIP22d,
          self.SOM22d,self.Xmod2d,self.VIP02d,self.SOM02d]) = self.detach_and_reattach([self.Y2mod2,self.VIP22,self.SOM22,
                                                                                        self.Xmod2,self.VIP02,self.SOM02])    

        if not self.one_scale:
            ([self.Y6mod2d,self.VIP62d,self.SOM62d,
              self.Y3mod2d,self.VIP32d,self.SOM32d]) = self.detach_and_reattach([self.Y6mod2,self.VIP62,self.SOM62,
                                                                              self.Y3mod2,self.VIP32,self.SOM32])

        prevY2mod = self.Y2mod.detach()
        (self.Xmod,self.VIP0,self.SOM0) = self.input_layer.forward(self.Y2, self.Y2mod2d, input_env)
        if self.one_scale:
            (self.Y2mod,self.VIP2,self.SOM2) = self.low_scale_layer.forward(self.low_scale, lower_ymod=self.Xmod,upper_ymod=None,horiz = self.Y2mod2d)
        else:
    ...
    def accessory(self):
        ...
        init = torch.autograd.grad(self.Z[self.action], [self.Y2mod,self.VIP2,self.SOM2,
        ...
        if not self.one_scale:
            init_2 = torch.autograd.grad(self.Z[self.action], [self.Y3mod,self.VIP3,self.SOM3,
